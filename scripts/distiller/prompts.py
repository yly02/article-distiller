...
- 默认读者是聪明的 AI 初学者。文章是把原文翻译好懂，不是另写评论；先判断体裁、列出原文要点清单，再一节一项写。
- 标题=高认知主体+人话结果。产品/研究先定义；分析用旧预期加例子；方法用失败模式；新闻先讲谁做了什么。不把散文腔套到产品稿，也不把分析/方法稿写成产品能力清单。
- 一节一项原文要点：产品节点名演示（改了什么、没改什么）；分析节讲清原文划界并保留钉子句；方法节写清步骤、闸门和对照。没看见的画面不要编。
- 速览严格 3 条，内容跟体裁走。产品：它是什么 / 多了哪一步 / 怎么用。研究：发布了什么 / 怎样工作 / 演示或评测证明了哪步。分析：发生了什么 / 原文怎么划界 / 结论落在哪。方法：要解决什么失败 / 方法哪一步 / 读者能照着做什么。recommendation_reason、takeaway_list、action_card 必须留空。
- 要点讲完再写读者能带走的一步。产品写设置/价格；研究写评测与分工；分析写判断落点；方法写可执行清单和不是生产文件的边界。演示、框架和步骤写够，安全和口径短写。
...
严格输出：
{
  ...
  "narrative_plan": {
    ...
    "opening_sequence": {"scene": "产品/研究写它是什么，分析写旧预期，方法写失败模式，新闻写真实细节", "turn": "产品写改了哪一步，研究写一次操作，分析写具体例子，方法写谁做了什么", "reveal": "产品写怎么用，研究写机制，分析写原文问题，方法写方法一句"},
    ...
    "chapter_system": {"archetype": "product|research|analysis|method|event|investigation|explainer", "throughline": "按原文要点推进的主线；产品按能力，研究按机制-演示-评测，分析按划界，方法按步骤闸门", "chapters": [{"section_id": "对应 sections[].id", "role": "进入|发现|解释|举证|转折|后果|选择|收束", "reader_need": "本节覆盖的原文要点，不必都写成问句", "advance": "相对上一节新增的能力、框架、步骤、演示、数字或限制", "evidence": "本节点名的演示、框架或材料", "handoff": "下一节为何必须接着出现；没有自然承接时留空"}]},
  ...
  "sections": [{
    ...
    "content": "本节只完成一个主要任务，对应原文一项要点；产品节点名演示，分析节讲清划界，方法节写清步骤和闸门，再写读者看哪里和限制",
...
- 产品页先定义再按原文能力拆演示；研究页先定义再给一次操作和机制；分析稿用旧预期加例子进入原文划界；方法稿用失败模式进入步骤和闸门。不要抢在首屏用抽象行业结论替读者总结。要点讲完再写读者能带走的一步；安全和口径短写。
...
def _editorial_review_prompt_for_modes(required_modes: tuple[str, ...]) -> str:
    ...
    focus = "先检查是否真正面向希望了解并进入 AI 领域的聪明初学者：首屏承诺清楚，首次术语可懂，读者能复述原文要点、关键演示或框架、以及能带走的一步。再检查标题是否是主体+人话结果、体裁骨架是否选对、产品是否点名演示、分析是否保留原文划界、方法是否写出闸门、官方附件是否补了设置/价格或方法骨架。quick_scan 必须是 3 条且总计不超过 180 字，内容跟体裁走；不得把重建案例写成逐字对话，不得另写独特见解评论。"
    voice_requirement = ""
    voice_report_fields = ""
    if mode == "full":
        voice_requirement = """
- 按写作总纲审，不另起一套标准。写作引擎是原文要点清单，不是独特见解；骨架必须跟体裁走。
- 速览必须恰好 3 条，内容跟体裁走；recommendation_reason 与 takeaway_list 必须为空。
- 前台像文章：产品/研究首屏给定义，分析给旧预期和例子，方法给失败模式，新闻给事件；审计话术只留内部字段。
- 产品节应点名演示，分析节应讲清原文划界，方法节应写出步骤和闸门。改图、失败版和收紧版要有前后对照；过程用视频。没看见的画面不要补写。
- 语气跟着原文体裁。不把危机散文腔套到产品稿，不把分析/方法稿写成产品能力清单，不抄对标站标题腔和长提示词模板。
