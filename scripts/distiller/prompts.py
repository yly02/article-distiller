...
- 默认读者是聪明的 AI 初学者。全文一个因果机制；标题先放高认知主体，再放材料支持的动作、冲突或完整口径数字。
- 首屏先让事情发生，再解释机制。正文不要用“原文/本文/当前材料”当主语。
- recommendation_reason、takeaway_list、action_card 必须留空；值得看的理由只写进 quick_scan 第二条。
...
def _editorial_review_prompt_for_modes(required_modes: tuple[str, ...]) -> str:
    ...
    if mode == "full":
        voice_requirement = """
- 按写作总纲审，不另起一套标准。
- 速览必须恰好 3 条，第二条承担“为什么值得看”；recommendation_reason 与 takeaway_list 必须为空。
- 前台像文章：首屏给场景和判断，审计话术只留内部字段。
