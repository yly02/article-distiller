...
写作标准是：真材实料为底，原文要点为骨，白话为形。目标读者是 B 端老板：用他们关心的判断、成本、能不能用来说原文，不改原文意思，也不句句点业务。写作引擎只有一套，细则以 [references/article-depth.md](references/article-depth.md) 为准：标题是主体加材料支持的人话结果，必须吸引人、有冲击力；一句导语加三条速览；一节对应原文一项要点；语气和骨架跟着原文体裁走。单产品按能力，发布会先全景后深挖，论文用日期或对照数字开场，商业动态短写机制。同源套件仍按单产品，官方研究报告并进论文，多口径速度数字拆开；不要把分析稿、方法稿写成产品能力清单，也不要把发布会或论文改成速查表。
...
3. 先判断体裁并列出原文要点清单，再写成白话章节。一节只讲一项要点：单产品节点名演示，发布会先全景后逐台深挖，论文节写清样本和指标，分析节讲清原文划界，方法节写清步骤和闸门，商业动态写清机制和两边评价。同源套件按任务难度拆，不要改成发布会；官方研究报告用对照数字开场；推理时延、墙钟和吞吐不是同一个“快”。不要为了“独特见解”把原文重写成另一篇文章。原文有的能力、框架、演示、数字、工具和限制默认都要覆盖；只有重复装饰或与要点无关时才能省略，并写明理由。材料足够时优先搭出 5-8 个有独立任务的章节。要点讲完再写读者能带走的一步；安全与口径短写，演示、框架和步骤写够。
...
5. 主编审校完成后统一运行中文语病、事实与结构、证据与媒体对账、HTML 渲染和浏览器验收。表格色板和缺数字叙事先确定性补齐；模型修复只处理仍卡住的硬阻断，提示词只送失败项。门禁失败也先落盘成稿，结构完整就继续渲染。
...
- 只交付深度文章 HTML。
- 不能基于空正文或明显残篇写作。
- 目标读者是 B 端老板。描述重点从他们会关心的方向写：值不值得跟、改了哪一步、花不花钱、能不能用。取景方法是先让一件会卡住的事发生，再落到原文真正改的那一步，然后按原文步骤讲完。这是取景，不是改题；不另立“服务业务”的主题，也不要目的性太强，句句点业务、周报或降本。日常工作只在原文真有对应场景时点一下。标题先放高认知主体，再放材料支持的人话结果，必须吸引人、有冲击力，让老板愿意点开，不空喊改变社会。冲击力来自反差、反常结果、具体动作或可核验规格，不要写成正确但没画面的过程句，也不要单独写推荐理由。一分钟速览固定 3 条，内容跟体裁走，让人一眼看见它是什么、为什么值得看、改了哪一步；文末不渲染打勾清单。
- 文章前台要像有主线的文章，不像审计报告；claim id、质量报告、抓取状态和研究账本只留在运行时。数字卡的 boundary、“这个数字不能说明什么”、语法含混等审计句不得进入 HTML；限制用人话写进正文。 图表下的“怎么读”“适用范围”同样不渲染。
- 页末来源区只显示来源名称，不展示 URL，链接不可点击跳转。有来源即可，不必告诉读者具体地址。
- 同一节里两张类比卡不要连着放。数字讲完就往下走，少写“作者认为”；全文类比通常 1-2 处，组件不能比正文抢戏。
- 客户可见文本必须通过中文语病检查；高置信错误做最小修改。主编通读必须逐句抽主干，不能只扫段落：冒号能否解说前项、分句主语是否中途更换、并列项是否同类。
- 产品发布若把“能看见的能力”当作卖点，必须采用能解释该能力的原页演示；交互表不能替代前后差异或操作视频。
- 介绍页链到的官方定价、模型文档或系统卡要实际读取；不能因为主文没写数字就停在“价格未知”。
- 解读是把原文翻译好懂，不另起炉灶；不省略原文要点和能解释要点的原页材料，也不把文章写成文绉绉的评论。不要用「别一上来就问」「不必专门说」这类对着读者下禁令的句子，把规则写成事情本身怎么发生。客户可见正文禁止「不是……是……」「不是……而是……」「而不是」这类对比句式；要纠正误解，改成两句正面陈述。
- 语气跟着原文体裁：单产品和研究用说明文，发布会先全景，论文用对照数字开场，分析用具体例子，方法用步骤和闸门，商业动态短写机制，危机散文才跟文学腔。同源套件仍按单产品，官方研究报告并进论文/临床，对标站栏目名不当写作体裁。不把对标站的标题腔、长提示词模板、会员栏或没看见的演示细节写进正文。发散只补官方文档、已看见的演示、参数、定价和原文自带的方法骨架。
